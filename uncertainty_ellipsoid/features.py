...
def main(
    ...
    try:
        ...
        with h5py.File(train_path, 'w') as f_train:
            f_train.create_dataset('world_coordinates', data=world_coordinates[train_indices], compression='gzip', chunks=True)
            f_train.create_dataset('pixel_coordinates', data=pixel_coordinates[train_indices], compression='gzip', chunks=True)
            f_train.create_dataset('depths', data=depths[train_indices], compression='gzip', chunks=True)
            f_train.create_dataset('uncertainty_sets', data=uncertainty_sets[train_indices], compression='gzip', chunks=True)
        ...
        with h5py.File(test_path, 'w') as f_test:
            f_test.create_dataset('world_coordinates', data=world_coordinates[test_indices], compression='gzip', chunks=True)
            f_test.create_dataset('pixel_coordinates', data=pixel_coordinates[test_indices], compression='gzip', chunks=True)
            f_test.create_dataset('depths', data=depths[test_indices], compression='gzip', chunks=True)
            f_test.create_dataset('uncertainty_sets', data=uncertainty_sets[test_indices], compression='gzip', chunks=True)
            f_test.attrs.update({
